...
export async function encryptMessage(
  ...
  const [senderPubKey, recipientPubKey] = await Promise.all([
    crypto.subtle.importKey("spki", b642buf(senderPublicKeyB64), RSA_PARAMS, false, ["encrypt"]),
    crypto.subtle.importKey("spki", b642buf(recipientPublicKeyB64), RSA_PARAMS, false, ["encrypt"]),
  ]);
